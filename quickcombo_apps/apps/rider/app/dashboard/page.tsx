...
export default function RiderDashboard() {
  ...
  const fetchDashboardData = async (t: string, isPolling = false) => {
    ...
    try {
      ...
      console.log('Incoming latestOrder:', latestOrder, 'prevLatestOrderId:', prevLatestOrderId.current, 'isFirstFetch:', isFirstFetch.current);
      if (latestOrder) {
        if (!isFirstFetch.current) {
          // Trigger notification if we have loaded at least once, and either we had no previous order or the ID is different
